({
        ...
        action.setCallback(this, function(response) {    
            ...
            if (state === "SUCCESS") {
                ...
                if(OrderObj == null){
                   ...
                   /*start demerging
                    * if(OrderObj.Product_Type_Order_Entry__c == 'Bond'){
                    ...
                    if(OrderObj.Product_Type_Order_Entry__c == 'MF'){
                      ...
                      if(OrderObj.Product_Type_Order_Entry__c === 'PE / AIF'){
                 component.set("v.showSecurityDisplay" , true); 
  ...
  /*  getFolioOnHolding : function(component, event, helper){
        ...
        if(orderEntry.Transaction_Type__c == 'Redemption' || orderEntry.Transaction_Type__c == 'Switch'){
            ...
            action.setCallback(this, function(response) {    
                ...
                if (state === "SUCCESS") {
                    var clientAssetList = response.getReturnValue();
                    if(clientAssetList == null){
                        helper.showToast("Client Asset not available2","Error");
                    }else{
                        folioList.push('-- None --');
                        for(var m=0 ; m < clientAssetList.length; m++){
                            folioList.push(clientAssetList[m].Folio_Number__c);
    ...
    loadMainScreen: function(component, event, helper){
        ...
        // if(isValid == 1){
        // Main Screen Loading
      /*demarge start  if(orderEntry.Product_Type_Order_Entry__c == 'MF'){
            var selectedClientRecord = component.get("v.selectedClientRecord");
            var listOfAllClients = component.get("v.filteredClientList");
            ...
            if(orderEntry.Transaction_Type__c == 'Redemption' || orderEntry.Transaction_Type__c == 'Switch'){
                ...
                action.setCallback(this, function(response) {    
                    ...
                    if (state === "SUCCESS") {
                        var clientAssetList = response.getReturnValue();
                        if(clientAssetList == null){
                            helper.showToast("Client Asset not available4","Error");
                        }else{
                            folioList.push('-- None --');
                            for(var m=0 ; m < clientAssetList.length; m++){
                                folioList.push(clientAssetList[m].Folio_Number__c);
        ...
        /*demarge start else if(orderEntry.Product_Type_Order_Entry__c === 'Bond'){
            component.set("v.isFirstScreen" , false); 
           ...
           /*demerge start  if(orderEntry.Product_Type_Order_Entry__c === 'PMS'){
                component.set("v.isFirstScreen" , false); 
                component.set("v.isBond", false);
        ...
        //Added By Aashay
         if(orderEntry.Product_Type_Order_Entry__c === 'PE / AIF'){
             ...
             if(orderEntry.Transaction_Type__c == 'Creation')
             {
               component.set("v.isCommitment" , true);  
             }
            else if(orderEntry.Transaction_Type__c == 'Creation and Drawdown')
            {
                component.set("v.isTransactionAmount" , true);
                component.set("v.isDrawdown" , true);
                component.set("v.isCommitment" , true);
            }
            else if(orderEntry.Transaction_Type__c == 'Drawdown or Withdraw')
            {
              component.set("v.isPEID", true);  
                component.set("v.isTransactionAmount" , true);
            }
            else if(orderEntry.Transaction_Type__c == 'Closeout')
            {
                component.set("v.isPEID", true);
            }
            }
    ...
    getToMain : function(component, event, helper) {
        ...
        component.set("v.isFirstScreen" , false); 
        component.set("v.loadPreviewScreen" , false); 
        /*demerge start if(orderEntry.Product_Type_Order_Entry__c == 'Bond'){
            component.set("v.isProuctTypeMF" , false);                
            component.set("v.isBond" , true);      
            component.set("v.isPMS" , false); 
            component.set("v.isPE" , false); 
        } demerge end*/
        /*demerge start else if(orderEntry.Product_Type_Order_Entry__c == 'MF') {
            component.set("v.isProuctTypeMF" , true);
            component.set("v.isBond" , false);      
            component.set("v.isPMS" , false); 
             component.set("v.isPE" , false); 
        }demerge end*/
         /*demerge start else if(orderEntry.Product_Type_Order_Entry__c == 'PMS') {
            component.set("v.isProuctTypeMF" , false);
            component.set("v.isBond" , false);      
            component.set("v.isPMS" , true); 
             component.set("v.isPE" , false); 
  ...
  /* demerge start fetchClientHolding: function(component, event, helper) {
        var orderEntry = component.get("v.orderEntry"); 
    ...
    */
   /*demerge start setFolioRedemption : function(component, event, helper) {
        var orderEntry = component.get("v.orderEntry"); 
        var selectedPicklistValue = component.get("v.selectedFolioOnSave");
        orderEntry.Folio__c = selectedPicklistValue; demerge end*/	
       ...
       /*demerge start var clientAssetList = component.get("v.clientAssetListRedemption"); 
...
/*demerge start    setUCC : function(component, event, helper) {
        var selectedUcc = component.get("v.selectedUcc");
    },demerge end */
 /*demerge start    setFolioPurchase : function(component, event, helper) {
        var orderEntry = component.get("v.orderEntry"); 
 ...
 /*dememrge start   handleComponentEvent : function(component, event, helper) {
...
/*demerge start    setValuesToFieldAmc : function(component, event, helper) {
        var orderEntry = component.get("v.orderEntry"); 
        var selectedAMCName1 = component.get("v.selectedAMCName1"); 
   ...
   /*demerge start   setValuesToFieldScheme : function(component, event, helper) {
        var orderEntry = component.get("v.orderEntry"); 
        var selectedSchemeObj = component.get("v.selectedSchemeObj"); 
        ...
        // orderEntry = component.get("v.orderEntry"); 
        if(orderEntry.Transaction_Type__c == 'Redemption')
        {
            var selectedClientAccountRecord = component.get("v.selectedClientAccountRecord");
            var selectedClientAccountId =   selectedClientAccountRecord.Id;
   ...
   /*demerge start  setValuesToFieldFromScheme : function(component, event, helper) {
      ...
      /*demerge startsetValuesToFieldToScheme : function(component, event, helper) {
    ...
    setValuesToField : function(component, event, helper) {      
        ...
        if(isValid == 1){
          ...
          /*demerge start  if(orderEntry.Redemption_Type__c == 'All Unit'&& orderEntry.Transaction_Type__c != 'Purchase'){
            ...
            /*demerge start else if(orderEntry.Redemption_Type__c == 'Partial Amount' && orderEntry.Transaction_Type__c != 'Purchase'){
                component.set("v.isPartialAmount" , true);
                component.set("v.isPartialUnit" , false);
                component.set("v.isRedemAllUnit" , false);
                component.set("v.isPurchaseAmount" , false);
            }demerege end*/
              /*demerge start  else if(orderEntry.Redemption_Type__c == 'Partial Unit' && orderEntry.Transaction_Type__c != 'Purchase'){
                component.set("v.isPartialUnit" , true);
                component.set("v.isPartialAmount" , false);
                component.set("v.isRedemAllUnit" , false);
                component.set("v.isPurchaseAmount" , false);
            }demerege end*/
                     /*demerge start else if(orderEntry.Transaction_Type__c == 'Purchase'){
                ...
                component.set("v.isBondBuy",false);
                                    ...
                                    if(orderEntry.Product_Type_Order_Entry__c == 'PE / AIF'){
                component.set("v.isPEScreen",true);
                component.set("v.isPreviewScreen" , true); 
                component.set("v.isBondBuy",false);
            }    
        }
 ...
 /*demerege start handleRedemptionChange : function(component, event, helper){       
        var orderEntry = component.get("v.orderEntry");
        var quantity;
        var totalAUA;
        var clientAssetList = component.get("v.clientAssetListPurchase");           
  ...
  /*demerge start  setValuesToPMSField : function(component, event, helper) {
        var orderEntry = component.get("v.orderEntry");
    ...
    /*demerge start  setFolioPMS : function(component, event, helper){
        var orderEntry = component.get("v.orderEntry");
